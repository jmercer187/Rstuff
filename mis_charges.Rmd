...
An examination of the effect if the state of Kentucky no longer required that any marijuana offense meant that an individual must be bought into custody. (Please see the Note on Assumptions)

```{r mj-decrim-calc, echo=FALSE}

removed_charges <- mj_mis %>%
  ungroup() %>%
  filter(str_detect(Charge, 'DEADLY') | str_detect(Charge, 'U/INFLUENCE') | str_detect(Charge, 'ROBBERY') | str_detect(Charge, 'EPO')) %>%
  distinct(Charge)
